...
const { getStableAmount, getNetwork, Networks, increaseTime } = require("./helpers/utils");
...
contract("ClaimVoting", async (accounts) => {
  ...
  async function initVoting(liquidityAmount, amount1, amount2) {
    ...
    if (amount1 > 0) {
      ...
      const toApproveOnePercent1 = await policyBookFacade1.getClaimApprovalAmount(USER1, toBN(bmiPriceInUSDT));
    ...
    if (amount2 > 0) {
      ...
      const toApproveOnePercent2 = await policyBookFacade2.getClaimApprovalAmount(USER2, toBN(bmiPriceInUSDT));
  ...
  async function initAppeal(claimIndex, policyBookFacade, user) {
    await fastVoteConfirmed(1, USER3, wei("1200"), wei("0"), toBN(PRECISION), false);
    ...
    const toApproveOnePercent = await policyBookFacade.getClaimApprovalAmount(user, toBN(bmiPriceInUSDT));
  ...
  async function voteAndExpose(suggestedClaimAmount, userPrivateKey, user) {
    ...
    await increaseTime(
      toBN(await claimingRegistry.anonymousVotingDuration(claimIndex))
        .plus(10)
        .toString()
    );
  ...
  describe("initializeVoting", async () => {
    ...
    it("reverts if not called by PolicyBook", async () => {
      ...
      await truffleAssert.reverts(
        claimVoting.initializeVoting(USER1, "", coverTokensAmount, toBN(bmiPriceInUSDT), false, {
          from: USER1,
        }),
        reason
      );
    ...
    it("reverts if claimer has no coverage", async () => {
      ...
      await truffleAssert.reverts(
        policyBook2.submitClaimAndInitializeVoting("", toBN(bmiPriceInUSDT), {
          from: USER2,
        }),
        reason
      );
    ...
    it("should initialize new claim", async () => {
      const res = await policyBook1.submitClaimAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 });
    ...
    it("should initialize new apppeal", async () => {
      await policyBook1.submitClaimAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 });
      await initAppeal(1, policyBookFacade1, USER1);
      const res = await policyBook1.submitAppealAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 });
    ...
    it("should initialize new claim after expired claim", async () => {
      ...
      await increaseTime(
        toBN(await claimingRegistry.validityDuration(1))
          .plus(10)
          .toString()
      );
    ...
    it("should initialize new appeal after expired appeal", async () => {
      await policyBook1.submitClaimAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 });
      let timestamp1 = await getBlockTimestamp();

      await initAppeal(1, policyBookFacade1, USER1);
      await policyBook1.submitAppealAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 });
      let timestamp2 = await getBlockTimestamp();
      ...
      await claimVoting.calculateResult(2, { from: USER4 });

      assert.equal(await policyRegistry.isPolicyActive(USER1, policyBook1.address), true);
      assert.equal(await claimingRegistry.claimStatus(2), ClaimStatus.EXPIRED);
      ...
      let timestamp3 = await getBlockTimestamp();
      await increaseTime(10);
      ...
      await claimingRegistry.withdrawLockedBMI(2, { from: USER1 });

      const claimsCount = await claimingRegistry.countClaims();
      assert.equal(claimsCount, 3);
      const claims = await claimVoting.listClaims(0, claimsCount, ListOption.ALL);
      ...
      assert.equal(toBN(claims[0][0][5]).toString(), coverage.toString());
      assert.equal(claims[0][0][6], timestamp1);
      assert.equal(claims[0][1], ClaimStatus.REJECTED);
      assert.equal(claims[0][2], 0);
      assert.equal(claims[0][3], 0);
      ...
      assert.equal(toBN(claims[1][0][5]).toString(), coverage.toString());
      assert.equal(claims[1][0][6], timestamp2);
      assert.equal(claims[1][1], ClaimStatus.EXPIRED);
      assert.equal(claims[1][2], 0);
      assert.equal(claims[1][3], 0);

      assert.equal(claims[2][0][0], 3);
      assert.equal(claims[2][0][1], USER1);
      assert.equal(claims[2][0][2], policyBook1.address);
      assert.equal(claims[2][0][3], "");
      assert.equal(claims[2][0][4], true);
      assert.equal(toBN(claims[2][0][5]).toString(), coverage.toString());
      assert.equal(claims[2][0][6], timestamp3);
      assert.equal(claims[2][1], ClaimStatus.PENDING);
      assert.equal(claims[2][2], 0);
      assert.equal(claims[2][3], 0);

      const claimResult = await claimVoting.getVotingResult(3);
  ...
  describe("encrypt vote", async () => {
    ...
    describe("anonymouslyVoteBatch()", async () => {
      ...
      it("should not successfully vote if voter has awaiting reception votes", async () => {
        ...
        await policyBook2.submitClaimAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER2 });
      ...
      it("should not successfully vote if anonymous voting is over", async () => {
        const timestamp = await getBlockTimestamp();
        await increaseTime(10 * 24 * 60 * 60); // 10 days
    ...
    describe("exposeVoteBatch()", async () => {
      ...
      it("should fail if amount exceeds coverage", async () => {
        ...
        increaseTime(
          toBN(await claimingRegistry.anonymousVotingDuration(2))
            .plus(10)
            .toString()
        );
  ...
  describe("vote for or against", async () => {
    ...
    describe("vote for", async () => {
      ...
      it("should expire appeal if no one voted for appeal", async () => {
        await initAppeal(1, policyBookFacade1, USER1);
        await policyBook1.submitAppealAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 });
        timestamp = await getBlockTimestamp();
  ...
  describe("calculateResult()", async () => {
    ...
    it("should calculate reward, public calculation (7)", async () => {
      ...
      await increaseTime(currentTime.toString());
    ...
    it("should expire the claim if no vote", async () => {
      await increaseTime(
        toBN(await claimingRegistry.votingDuration(1))
          .plus(10)
          .toString()
    ...
    it("should only claim same policy after appeal expires", async () => {
      ...
      const toApproveOnePercent = await policyBookFacade1.getClaimApprovalAmount(USER1, toBN(bmiPriceInUSDT));
      await bmi.approve(claimVoting.address, 0, { from: USER1 });
      ...
      await truffleAssert.reverts(
        policyBook1.submitAppealAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 }),
        reason
      );
  ...
  describe("receiveVoteResultBatch()", async () => {
    ...
    it.only("should receive voter result (yes, majority, accepted) // withdraw reward ready", async () => {
      ...
      await increaseTime(toBN(WITHDRAWAL_PERIOD).plus(10).toString());
    ...
    it.only("should receive voter result (yes, majority, accepted) // miss withdraw reward, request with no new vote", async () => {
      const staked = [wei("4000"), wei("3000"), wei("1000"), wei("1000")];
      const reputations = [toBN(PRECISION), toBN(PRECISION), toBN(PRECISION), toBN(PRECISION)];
      ...
      await increaseTime(toBN(WITHDRAWAL_PERIOD).plus(READY_TO_WITHDRAW_PERIOD).plus(10).toString());
    ...
    it.only("should receive voter result (yes, majority, accepted) // miss withdraw reward, request with new vote", async () => {
    ...
    it("should receive correct voter reputation", async () => {
      ...
      await bmi.transfer(USER2, wei("1000000"), { from: USER1 });
      const toApproveOnePercent2 = await policyBookFacade1.getClaimApprovalAmount(USER2, toBN(bmiPriceInUSDT));
      await bmi.approve(claimVoting.address, 0, { from: USER2 });
      await bmi.approve(claimVoting.address, toApproveOnePercent2, { from: USER2 });
      await policyBook1.submitClaimAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER2 });
  ...
  describe("claimingRegistry.withdrawLockedBMI()", async () => {
    ...
    it("should unlock BMI successfully (claim expired)", async () => {
      await fastVoteConfirmed(1, USER3, wei("1200"), coverTokensAmount, toBN(PRECISION), true);
      await fastVoteConfirmed(1, USER4, wei("10000"), coverTokensAmount, toBN(PRECISION), true);
      await increaseTime(
        toBN(await claimingRegistry.validityDuration(1))
    ...
    it("should unlock BMI successfully (appeal expired)", async () => {
      await initAppeal(1, policyBookFacade1, USER1);
      await policyBook1.submitAppealAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 });
      ...
      await increaseTime(
        toBN(await claimingRegistry.validityDuration(2))
          .plus(10)
          .toString()
      );

      const claimantBalanceBefore = await bmi.balanceOf(USER1);
      const lockedBMI = toBN((await claimVoting.getVotingResult(2))[1]);
