...
contract("UserLeveragePool", async (accounts) => {
  ...
  describe("withdrawLiquidity", async () => {
    ...
    it("reverts if there is ongoing claim procedure", async () => {
      ...
      const toApproveOnePercent = await policyBookFacade.getClaimApprovalAmount(USER1, toBN(bmiPriceInUSDT));
      ...
      await policyBookMock.submitClaimAndInitializeVoting("", toBN(bmiPriceInUSDT), { from: USER1 });
