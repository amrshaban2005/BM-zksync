...
const { getStableAmount, getNetwork, Networks, increaseTime, increaseTimeTo } = require("./helpers/utils");
...
contract("ClaimingRegistry", async (accounts) => {
  ...
  describe("submitClaim()", async () => {
    ...
    it("should submit claim on EXPIRED claim", async () => {
      const id = (
      ...
      await setCurrentTime(
        toBN(timestamp)
          .plus(await claimingRegistryMock.validityDuration(id))
          .plus(10)
      );
      ...
      assert.equal(await claimingRegistryMock.isClaimAppeal(2), false);

      assert.equal(await claimingRegistryMock.claimStatus(1), ClaimStatus.EXPIRED);
      assert.equal(await claimingRegistryMock.isClaimAppeal(1), false);
    });
    ...
    it("should submit appeal on EXPIRED appeal", async () => {
      await claimingRegistryMock.submitClaim(USER1, policyBook1.address, "", coverTokensAmount, false, {
        from: CLAIM_VOTING,
      });
      await claimingRegistryMock.updateStatus(USER1, policyBook1.address, ClaimStatus.REJECTED_CAN_APPEAL);
      ...
      assert.equal(await claimingRegistryMock.isClaimAppeal(3), true);

      assert.equal(await claimingRegistryMock.claimStatus(2), ClaimStatus.EXPIRED);
      assert.equal(await claimingRegistryMock.isClaimAppeal(2), true);
    ...
    it("shouldn't be able to submit claim on ACCEPTED appeal", async () => {
      await setCurrentTime(toBN(timestamp).plus(10));
      await claimingRegistryMock.submitClaim(USER1, policyBook1.address, "", coverTokensAmount, false, {
        from: CLAIM_VOTING,
      });
      await claimingRegistryMock.updateStatus(USER1, policyBook1.address, ClaimStatus.REJECTED_CAN_APPEAL);
    ...
    it("should not submit claim on EXPIRED appeal", async () => {
    ...
    it("should not submit appeal on reappealed appeal (post EXPIRED appeal)", async () => {
      setCurrentTime(toBN(timestamp).plus(10));
      await claimingRegistryMock.submitClaim(USER1, policyBook1.address, "", coverTokensAmount, false, {
        from: CLAIM_VOTING,
      });
  ...
  describe("expireClaim()", async () => {
    ...
    it("should expire the claim", async () => {
      ...
      await increaseTime(
        toBN(await claimingRegistryMock.validityDuration(id))
          .plus(10)
          .toString()
      );
  ...
  describe("hasProcedureOngoing()", async () => {
    ...
    it("return true inside (the Claim is Denied + 7 days for Appeal)", async () => {
      ...
      await increaseTime(5 * 24 * 60 * 60 + 10);
      assert.equal(await policyBook1.hasProcedureOngoing(policyBook1.address), true);
    });
    it("return true inside (the Claim is Approved + 8 days to receive the payout)", async () => {
      ...
      await increaseTime(7 * 24 * 60 * 60 + 10);
      assert.equal(await policyBook1.hasProcedureOngoing(policyBook1.address), true);
    });
    it("return true inside (Claim is Approved, Claimant missed the payout window, but Policy has not yet Ended + days until the End of the policy (Ended status) + 8 days to receive the payout)", async () => {
      ...
      await increaseTime(toBN(347).times(24).times(60).times(60).plus(10).toString()); // 1 day before end policy
      assert.equal(await policyRegistryMock.isPolicyActive(USER1, policyBook1.address), true);

      await claimingRegistryMock.requestClaimWithdrawal(1, { from: USER1 });

      await increaseTime(
        toBN(WITHDRAWAL_PERIOD)
          .plus(7 * 24 * 60 * 60)
          .plus(10)
          .toString()
  ...
  describe("requestClaimWithdrawal()", async () => {
    ...
    it("should not request withdraw if policy expired", async () => {
      ...
      await increaseTime(toBN(timestamp).plus(toBN(365).times(24).times(60).times(60)).plus(10).toString());
  ...
  describe("withdrawClaim()", async () => {
    ...
    it("should not withdraw claim is not requested or expired", async () => {
      await increaseTime(toBN(READY_TO_WITHDRAW_PERIOD).plus(10).toString());
  ...
  describe("claimInfo()", async () => {
    ...
    it("should return valid claim info", async () => {
      const dateSubmitted = toBN(await time.latest());
      ...
      assert.closeTo(toBN(claim[3]).toNumber(), dateSubmitted.toNumber(), 1);
    ...
    it("should be public", async () => {
      ...
      await increaseTime(
        toBN(await claimingRegistryMock.anyoneCanCalculateClaimResultAfter(id))
          .plus(10)
          .toString()
      );
