...
contract("PolicyBookAdmin", async (accounts) => {
  ...
  describe("updateImageUriOfClaim", async () => {
    ...
    beforeEach("setup", async () => {
      ...
      const toApproveOnePercent = await policyBookFacades[0].getClaimApprovalAmount(USER1, toBN(bmiPriceInUSDT));
      await bmi.approve(claimVoting.address, toApproveOnePercent, { from: USER1 });
      await policyBooks[0].submitClaimAndInitializeVoting(URI1, toBN(bmiPriceInUSDT), { from: USER1 });
